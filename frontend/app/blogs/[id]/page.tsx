...
export default function BlogDetailPage() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || "Unable to load blog details.");
        }

        setPost(data.data);
      } catch (error: any) {
        setError(error.message || "Unable to load blog details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
        try {
            ...
            const token = typeof window !== "undefined" ? window.localStorage.getItem("blogify-token") : null;
            const response = await fetch(`${API_URL}/v1/comments/post/${post._id}`, {
              headers: {
                ...(token ? { Authorization: `Bearer ${token}` } : {}),
              },
            });
    ...
    fetchComments();
    // setComments([
    //   { id: "c1", author: "Sara Khan", text: "Mazaydaar likha hai, bohat informative hai!", createdAt: post.createdAt },
    //   { id: "c2", author: "Ali Ahmed", text: "Yeh blog mere liye bohat helpful tha. Shukriya.", createdAt: post.updatedAt },
    // ]);
  }, [post, savingComment]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(error.message || "Could not delete the blog.");
    }
  };
  ...
  const handleCommentSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const token = typeof window !== "undefined" ? window.localStorage.getItem("blogify-token") : null;
      const user = typeof window !== "undefined" ? window.localStorage.getItem("blogify-user") : null;
      const commentAuthor = user ? JSON.parse(user).name : "Anonymous";
      const response = await fetch(`${API_URL}/v1/comments`, {
        ...
        body: JSON.stringify({ postId: post._id, text: commentText.trim(), userId: commentAuthor }),
      });
      const data = await response.json();
  ...
  const image = "https://images.unsplash.com/photo-1515378791036-0648a3ef77b2?auto=format&fit=crop&w=1200&q=80";
    ...
    <main className="min-h-screen bg-slate-50 px-6 py-10 dark:bg-slate-950">
      <div className="mx-auto max-w-6xl space-y-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">Blog details</p>
            <h1 className="mt-4 text-4xl font-semibold text-slate-950 dark:text-white">{post?.title || "Loading blog..."}</h1>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link href="/blogs" className="rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800">
              Back to blogs
            </Link>
            <Link href={`/blogs/${id}/edit`} className="rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-slate-200">
              Edit post
            </Link>
            <button
              type="button"
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            <div className="overflow-hidden rounded-4xl border border-slate-200 bg-white shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
              <img src={image} alt={post.title} className="h-80 w-full object-cover" />
              <div className="p-8">
                ...
                <p className="mt-6 whitespace-pre-wrap text-lg leading-8 text-slate-700 dark:text-slate-300">{post.content}</p>
              </div>
            </div>
            ...
            <div className="grid gap-10 lg:grid-cols-[2fr_1fr]">
              <div className="space-y-6">
                <div className="rounded-4xl border border-slate-200 bg-white p-8 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
                  ...
                  <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">Share feedback on this blog or read through the latest responses.</p>
                  ...
                  <form onSubmit={handleCommentSubmit} className="mt-6 space-y-4">
                    ...
                    >
                      {savingComment ? "Posting comment..." : "Post comment"}
                    </button>
                  </form>
                </div>
              ...
              <div className="rounded-4xl border border-slate-200 bg-white p-8 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
                <h3 className="text-xl font-semibold text-slate-950 dark:text-white">Quick stats</h3>
